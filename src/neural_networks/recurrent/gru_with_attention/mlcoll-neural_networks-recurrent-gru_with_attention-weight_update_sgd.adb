...
begin
    ...
    for I in Model.Ba_In'Range loop
        Model.Ba_In (I) := Model.Ba_In (I) - (Gradient.Ba_In (I) * LR);
    end loop;

    for I in Model.A_In'Range loop
        Model.A_In (I) := Model.A_In (I) - (Gradient.A_In (I) * LR);
    end loop;

    for I in Model.Wa_In'Range (1) loop
        for J in Model.Wa_In'Range (2) loop
            Model.Wa_In (I, J) := Model.Wa_In (I, J) - (Gradient.Wa_In (I, J) * LR);
        end loop;
    end loop;

    for I in Model.Wb_In'Range (1) loop
        for J in Model.Wb_In'Range (2) loop
            Model.Wb_In (I, J) := Model.Wb_In (I, J) - (Gradient.Wb_In (I, J) * LR);
        end loop;
    end loop;

    for I in Model.Wa_Rec'Range (1) loop
        for J in Model.Wa_Rec'Range (2) loop
            Model.Wa_Rec (I, J) := Model.Wa_Rec (I, J) - (Gradient.Wa_Rec (I, J) * LR);
        end loop;
    end loop;
