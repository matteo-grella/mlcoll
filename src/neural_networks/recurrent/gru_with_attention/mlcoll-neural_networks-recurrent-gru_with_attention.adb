...
package body MLColl.Neural_Networks.Recurrent.GRU_With_Attention is
    ...
    begin
        ...
        for T in GRU_Structure.First_Sequence_Index .. GRU_Structure.Last_Sequence_Index loop
            ...
            GRU_Structure.Sequence_Attention_Layers (T) := new Real_Array
              (Index_Type'First .. Index_Type'First + Index_Type (Hidden_Layer_Size) -  1); 
            -- Useless for "REC" variant
            
            GRU_Structure.Sequence_Attention_Scores (T) := new Real_Array
              (GRU_Structure.First_Sequence_Index .. GRU_Structure.First_Sequence_Index + T);
            
            GRU_Structure.Sequence_Attention_Hidden_Layers_B (T) := new Real_Array
              (Index_Type'First .. Index_Type'First + Index_Type (Hidden_Layer_Size) -  1);
            
            GRU_Structure.Sequence_Attention_Hidden_Layers_A (T) := new Real_Array_Access_Array
              (GRU_Structure.First_Sequence_Index .. GRU_Structure.First_Sequence_Index + T);
            
            for I in GRU_Structure.First_Sequence_Index .. GRU_Structure.First_Sequence_Index + T loop
                GRU_Structure.Sequence_Attention_Hidden_Layers_A (T) (I) := new Real_Array
                  (Index_Type'First .. Index_Type'First + Index_Type (Hidden_Layer_Size) -  1);  
            end loop;
        end loop;
    ...
    begin
        ...
        -- Attention Layer
        Free (GRU_Structure.Sequence_Attention_Layers);
        Free (GRU_Structure.Sequence_Attention_Scores);
        Free (GRU_Structure.Sequence_Attention_Hidden_Layers_B);
        Free (GRU_Structure.Sequence_Attention_Hidden_Layers_A);
