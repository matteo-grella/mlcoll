...
begin
    ...
    for I in Model.W_In'Range (1) loop
        if Gradient.Input_Non_Zero(I) then -- optimization
            for J in Model.W_In'Range (2) loop
                declare
                    Avg_GW_In : constant Real :=  Gradient.W_In(I, J) / Real(Gradient.Count);
                begin
                    Model.Wm_In (I, J) := (Beta1 * Model.Wm_In (I, J)) + (Beta1_Inv * Avg_GW_In);
                    Model.Wv_In (I, J) := (Beta2 * Model.Wv_In (I, J)) + (Beta2_Inv * Avg_GW_In * Avg_GW_In);
    ...
    for I in Model.Bm_In'Range loop
        declare
            Avg_GB_In : constant Real := Gradient.B_In (I) / Real(Gradient.Count);
        begin
            Model.Bm_In (I) := (Beta1 * Model.Bm_In (I)) + (Beta1_Inv * Avg_GB_In );
            Model.Bv_In (I) := (Beta2 * Model.Bv_In (I)) + (Beta2_Inv * Avg_GB_In * Avg_GB_In  );
    ...
    for I in Model.Wm_Out'Range (1) loop
        for J in Model.Wm_Out'Range (2) loop
            declare
                Avg_GW_Out : constant Real := Gradient.W_Out (I, J) / Real(Gradient.Count);
            begin
                Model.Wm_Out (I, J) := (Beta1 * Model.Wm_Out (I, J)) + (Beta1_Inv * Avg_GW_Out);
                Model.Wv_Out (I, J) := (Beta2 * Model.Wv_Out (I, J)) + (Beta2_Inv * Avg_GW_Out * Avg_GW_Out);
    ...
    for I in Model.Bm_Out'Range loop
        declare
            Avg_GB_Out : constant Real := Gradient.B_Out (I) / Real(Gradient.Count);
        begin
            Model.Bm_Out (I) := (Beta1 * Model.Bm_Out (I)) + (Beta1_Inv * Avg_GB_Out);
            Model.Bv_Out (I) := (Beta2 * Model.Bv_Out (I)) + (Beta2_Inv * Avg_GB_Out * Avg_GB_Out);
