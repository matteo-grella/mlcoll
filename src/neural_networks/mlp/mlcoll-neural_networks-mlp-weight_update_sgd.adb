...
begin
    ...
    for I in Model.W_In'Range (1) loop
        if Gradient.Input_Non_Zero(I) then
            for J in Model.W_In'Range (2) loop
                Model.W_In (I, J) := Model.W_In (I, J) - (LR * (Gradient.W_In (I, J) / Real(Gradient.Count)));
            end loop;
        end if;
    end loop;
    ...
    for I in Model.B_In'Range loop
        Model.B_In (I) := Model.B_In (I) - (LR * (Gradient.B_In (I)  / Real(Gradient.Count)));
    end loop;

    -- Update Output Weight
    for I in Model.W_Out'Range (1) loop
        for J in Model.W_Out'Range (2) loop
            Model.W_Out (I, J) := Model.W_Out (I, J) - (LR * (Gradient.W_Out (I, J)  / Real(Gradient.Count)));
        end loop;
    end loop;

    -- Update Output Bias
    for I in Model.B_Out'Range loop
        Model.B_Out (I) := Model.B_Out (I) - (LR * (Gradient.B_Out (I) / Real(Gradient.Count)));
    end loop;
