...
package MLColl.Neural_Networks.MLP is
    ...
    type Gradient_Type
      (Input_Layer_Last  : Index_Type;
       ...
       Output_Layer_Last : Index_Type) is 
       record
           B_In : Real_Array
    ...
    type NN_Structure_Type
      (Input_Layer_Size        : Positive;
       ...
       record
           ...
           Sparse_Input : Index_Type_Vectors.Vector; -- SPARSE
            
           Input_Layer  : Real_Array (Index_Type'First .. Input_Layer_Last)
             := (others => 0.0);

           Hidden_Layer : Real_Array (Index_Type'First .. Hidden_Layer_Last)
             := (others => 0.0);

           Activated_Hidden_Layer : Real_Array (Index_Type'First .. Hidden_Layer_Last)
             := (others => 0.0);

           Output_Layer : Real_Array (Index_Type'First .. Output_Layer_Last)
             := (others => 0.0);

           Normalized_Output : Real_Array (Index_Type'First .. Output_Layer_Last)
             := (others => 0.0);
        
           Output_Error      : Real_Array (Index_Type'First .. Output_Layer_Last)
             := (others => 0.0);
        
           Input_Error       : Real_Array (Index_Type'First .. Input_Layer_Last)
             := (others => 0.0);
        
           Input_Relevance   : aliased Real_Array
             (Index_Type'First .. Input_Layer_Last) 
             := (others => 0.0);
    ...
    function Make_Gradient
      (Input_Layer_Size    : in     Positive;
       ...
       Output_Layer_Size   : in     Positive) return MLColl.Neural_Networks.MLP.Gradient_Type
      with Inline;
    
    function Make_NN_Structure
      (Input_Layer_Size    : in     Positive;
       Hidden_Layer_Size   : in     Positive;
       Output_Layer_Size   : in     Positive) return MLColl.Neural_Networks.MLP.NN_Structure_Type
      with Inline;
        
    procedure Print_NN_Structure_Size
      (NN_Structure : NN_Structure_Type);
    ...
    procedure Forward
      (Model                  : in     Model_Type;
       NN_Structure           : access NN_Structure_Type);
    ...
    function Calculate_Binary_Output_Error
      (Model                           : in     Model_Type;
       ...
       Constraint_Error                : in     Real := 0.0) return Real with
      Pre => NN_Structure.Output_Error'Length = NN_Structure.Normalized_Output'Length;
    ...
    function Calculate_Output_Error
      (Model                           : in     Model_Type;
       ...
       Constraint_Error                : in     Real := 0.0) return Real;
    
    procedure Backward
      (Model                : in out Model_Type;
       NN_Structure         : in out NN_Structure_Type;
       Gradient             : in out Gradient_Type;
       Accumulate_Gradients : in Boolean := False);
